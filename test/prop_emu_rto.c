...
prop_emu_matches_asm(struct theft *t, void *arg1, void *arg2, void *arg3) {
    ...
    double emu_out = fma_emu_rto(a, b, c);
    ...
    if (isnan(emu_out) && !isnan(asm_out)) {
...
int main() {
    // Check known discrepant input.
    double a = 1.2;
    double b = 2.3;
    double c = 3.4;

    // Rounding to nearest twice, at double precision, should disagree
    // with `vfmadd213sd`.
    assert(fma_asm(a, b, c) != (b*a + c));

    double asm_out = fma_asm(a, b, c);
    double emu_out = fma_emu_mp(a, b, c);

    // Emulated fused multiply-add should agree with the intrinsic.
    assert(asm_out == emu_out);
